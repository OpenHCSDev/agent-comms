...
- **Injected-message cursor: bounded historical coverage only.** Independent
  exact1972 review found missing post-return owner/full-attempt equality and
  an eager full-log scan before a 100-initial limit (NON-CLEAN, preserved).
  The successor compares the complete binding to the reserved SQL identity
  and native journal *before* recording context. Historical read-only coverage
  now refuses over an 8 MiB pre-guard bus cap, 1000 verified rows and 100
  initials, with a best-effort 250 ms scan check; it verifies sealed full-N/K
