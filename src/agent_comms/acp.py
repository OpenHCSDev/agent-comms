...
from .image_inputs import ImageInput, prompt_images
...
class CommsAgent:
    ...
    async def prompt(self, session_id: str, prompt: list[Any], **kwargs: Any) -> PromptResponse:
        ...
        if "compact" in options:
            instructions = options.get("compact")
            if instructions is not None and not isinstance(instructions, str):
                raise RequestError.invalid_params(
                    {"reason": "compact instructions must be a string or null"}
                )
            result = (
                await self._proxies[session_id].request("compact", instructions=instructions)
                if session_id in self._proxies
                else await self.compact_context(session_id, instructions)
            )
            ...
            inbox.put_nowait(
                {
                    ...
                    # Both deliveries steer: Pi injects at the next safe boundary
                    # (after the current step/tool), not after the whole run.
                    # "queue" only defers the local echo until it is delivered.
    ...
    async def compact_context(
        ...
        """Compact an idle thread as owner-authored maintenance with a visible lifecycle."""
        ...
        async with self._turn_locks.setdefault(session_id, asyncio.Lock()):
            ...
            self._comms.begin_turn(thread_name, turn_id, "Compacting context")
            self._comms.set_activity(thread_name, ActivityState.WORKING, "Compacting context")
            ...
            try:
                ...
                if result.get("ok"):
                    ...
                    counts = (
                        f" ({before:,} → approximately {after:,} tokens)"
                    ...
                    await self._emit_event(
                        session_id,
                        {
                            ...
                            "text": (
                                f"[compaction error] "
                                f"{result.get('error') or 'Compaction failed.'}"
                            ),
                        },
                    )
                return result
    ...
    async def _emit_input_started(self, session_id: str, text: str | None) -> None:
        await self._runtime.session_update(
            session_id=session_id,
            update=AgentMessageChunk(
                session_update="agent_message_chunk",
                content=TextContentBlock(type="text", text=""),
                field_meta={"agentComms": {"inputStarted": {"text": text}}},
            ),
        )
            ...
            async for event in backend.stream_agent_events(
                ...
                **image_options,
                ...
                if kind == "input_started":
                    item = self._queued_inputs.get(session_id, {}).pop(event.get("id") or "", None)
                    await self._emit_input_started(
                        session_id, item.text if item and item.echo else None
                    )
                    await self._emit_queue_state(session_id)
                if (
                    kind == "done"
            ...
            if terminal_ok is None and goal is not None and goal.active:
                ...
                if (
                    ...
                    self._comms.block_goal_after_failed_turn(
                        ...
                        expected_worktree=thread.worktree,
                        diagnostic=(
                            "Backend turn ended without a result; "
