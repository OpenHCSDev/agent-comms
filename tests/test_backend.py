...
""",
    ...
    async def test_stream_failure_reaps_backend_and_steering_task(self, tmp_path):
        ...
        assert failed["ok"] is False and "stream failed" in failed["text"]
    ...
    async def test_caller_cancellation_reaps_backend_and_steering_task(self, tmp_path):
        ...
        assert queue.get_nowait() == "[peer] pending"
        assert queue.empty()
    ...
    async def test_failed_or_aborted_compaction_keeps_prior_epoch(self, tmp_path, compaction):
        ...
        assert [e["context_used"] for e in events if e["type"] == "agent_info"] == [250, 250]
    ...
    async def test_compaction_invalidates_old_usage_and_accepts_new_lower_usage(
        ...
        events = await _rpc_events(tmp_path, records)
        assert [e["context_used"] for e in events if e["type"] == "agent_info"] == (
            [None, 250, None, 40, 30] if post_compaction else [None, 250, None, None]
        )
...
""",
        ...
        assert '"streamingBehavior": "followUp"' in steering_path.read_text()
        assert any(
...
""",
        ...
        assert stalled["retryable"] is True and stalled["replay_safe"] is True
        assert stalled["side_effects_possible"] is False
...
""",
    ...
    async def test_retry_recovery_requires_substantive_model_progress(self, tmp_path):
        ...
        assert states[-1]["side_effects_possible"] is False
...
""",
    ...
    async def test_watchdog_restores_only_forwarded_inputs_that_never_started(
...
""",
        ...
        if started:
            assert started_events == [{"type": "input_started", "id": "queued-1"}]
            assert queue.empty()
        else:
